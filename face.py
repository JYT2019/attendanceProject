...
import os
import requests
from attendanceStatic import buildexcel
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        MainWindow.resize(self,1920, 1080)
        self.setupUi(self)
    ...
    def get_userinfo_fromapifile(self):
        ...
        for user in self.user_info['result']:
            ...
            ###############################################
            #print(face_recognition.load_image_file(user["avatar"]))
            #print(face_recognition.face_encodings(
                    #face_recognition.load_image_file(user["avatar"])))
            self.person.append(
                        ...
                        if match[i]:
                            ...
                            if self.names['is_signto%s'%i] > self.rectimes:
                                ...
                                if (self.names['time_last%s'%i] - self.names['time_first%s'%i]) > self.sign_between:
                                    ...
                                    attendanceinfo.append(this_name)
                                    attendanceinfo.append(this_sex)
                                    attendanceinfo.append(this_age)
                                    attendanceinfo.append(this_rank)
                                    attendanceinfo.append(this_department)
                                    attendanceinfo.append(this_slogan)
                                    #this_time = time.time()
                                    #good = this_time.strftime("%Y-%m-%d %H:%M:%S %A", this_time)
                                    #this_date = time.time()
                                    #attendanceinfo.append(self.this_time)
                                    #attendanceinfo.append(self.this_date)
                                    buildexcel(attendanceinfo)
                                else:
                                    pass
